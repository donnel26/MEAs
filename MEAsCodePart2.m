...
%% 
dS_db_values = zeros(N_j, length(b_values), length(Q_values), N_sim); 
dS_dq_values = zeros(N_j, length(b_values), length(Q_values), N_sim);  
    ...
    for j = 1:length(Q_values)
        for i = 1:(length(b_values) - 1)
            dS_db_values(:, i, j,s) = (1 / mu_temp) * ((gamma_0_temp + gamma_1_temp * Z_simulated) + ...
                theta_hat_temp * dsigma_db_values(i, j));
        end
    end
    ...
    for i = 1:length(b_values)
        for j = 1:(length(Q_values) - 1)
            dS_dq_values(:, i, j,s) = (1 / mu_temp) * ((phi_0_temp + phi_1_temp * Z_simulated) + ...
                theta_hat_temp * dsigma_dq_values(i, j)); % Corrected index for dsigma_dq_values
        end
    end
end
...
for col = 1:(length(b_values) - 1)
    stats_dS_db(1, col) = mean(sorted_dS_db_values(:, col));  % Mean
    stats_dS_db(2, col) = prctile(sorted_dS_db_values(:, col), 25);  % 25th percentile
    stats_dS_db(3, col) = prctile(sorted_dS_db_values(:, col), 75);  % 75th percentile
end

% Compute summary statistics for dS/dq
for col = 1:(length(Q_values) - 1)
    stats_dS_dq(1, col) = mean(sorted_dS_dq_values(:, col));  % Mean
    stats_dS_dq(2, col) = prctile(sorted_dS_dq_values(:, col), 25);  % 25th percentile
    stats_dS_dq(3, col) = prctile(sorted_dS_dq_values(:, col), 75);  % 75th percentile
end
...
disp(CI_dS_dq);
  ...
  %%% Now your final output summary follows %%%
disp('Final Output Summary');
disp(['Theta_hat (θhat_0): ', num2str(theta_hat)]);
disp(['Constant (intercept): ', num2str(constant)]);
disp(['Share of pay=1 (sigma): ', num2str(sigma)]);  
